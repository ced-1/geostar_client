...
function findelt(){
	...
	for (var i=0;i<splitelt.length;i++){		// Pour chaque éléments
		if (!(regexspace.test(splitelt[i]))){				// Verification que l'element peut être valide à la requete
			eltlist.push(splitelt[i]);	// On place tout les éléments dans une liste
			}
		}
	var res = $.ajax({	// Requête POST contenant un JSON contenant la liste d'element et la liste des type envoyé au serveur
		type: 'POST',
